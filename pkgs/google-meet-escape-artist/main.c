...
extern CFArrayRef SLSCopySpacesForWindows(int connection, int selector,
...
bool window_is_sticky(uint32_t wid) {
  ...
  CFArrayRef window_list_ref = CFArrayCreate(
      NULL, (const void **)&window_id_ref, 1, &kCFTypeArrayCallBacks);
  CFArrayRef space_list_ref =
      SLSCopySpacesForWindows(_CGSDefaultConnection(), 0x7, window_list_ref);
  if (!space_list_ref)
    goto err;
...
void moveWindowToFarthestCorner(AXUIElementRef window, CGRect windowBounds,
  ...
  CGPoint corners[4] = {
      ...
      {screenSize.width - windowBounds.size.width - 15,
       screenSize.height - windowBounds.size.height - 15} // Bottom-right
  };
  ...
  AXValueRef positionValue =
      AXValueCreate(kAXValueCGPointType, &targetPosition);
...
CGEventRef mouseEventCallback(CGEventTapProxy proxy, CGEventType type,
  ...
  if (type == kCGEventMouseMoved) {
    ...
    if (CGEventSourceFlagsState(kCGEventSourceStateHIDSystemState) &
    ...
    if (mouseLocation.x >= windowBounds.origin.x - deltaX &&
        mouseLocation.x <=
            windowBounds.origin.x + windowBounds.size.width + deltaX &&
        mouseLocation.y >= windowBounds.origin.y - deltaY &&
        mouseLocation.y <=
            windowBounds.origin.y + windowBounds.size.height + deltaY) {
...
AXUIElementRef findChromeStickyWindow() {
  ...
  for (CFIndex i = 0; i < CFArrayGetCount(windowList); i++) {
    ...
    CFStringRef ownerName =
        CFDictionaryGetValue(windowInfo, kCGWindowOwnerName);

    // Check if the window belongs to Google Chrome
    if (CFStringCompare(ownerName, CFSTR("Google Chrome"), 0) ==
        kCFCompareEqualTo) {
      uint32_t windowID;
      CFNumberGetValue(CFDictionaryGetValue(windowInfo, kCGWindowNumber),
                       kCFNumberIntType, &windowID);

      // Check if the window is sticky
      if (window_is_sticky(windowID)) {
        pid_t pid;
        CFNumberGetValue(CFDictionaryGetValue(windowInfo, kCGWindowOwnerPID),
                         kCFNumberIntType, &pid);
        AXUIElementRef appRef = AXUIElementCreateApplication(pid);

        // Get the list of windows for the application
        CFArrayRef appWindows;
        AXUIElementCopyAttributeValue(appRef, kAXWindowsAttribute,
                                      (CFTypeRef *)&appWindows);
        if (appWindows && CFArrayGetCount(appWindows) > 0) {
          chromeStickyWindow =
              (AXUIElementRef)CFArrayGetValueAtIndex(appWindows, 0);
...
int main(void) {
  ...
  CFMachPortRef eventTap = CGEventTapCreate(
      ...
      eventMask, mouseEventCallback, chromeWindow);
  ...
  printf("Listening for mouse movements near the sticky Chrome window. Hold "
         "Ctrl to prevent movement. Press Ctrl+C to exit.\n");
  CFRunLoopRun();
